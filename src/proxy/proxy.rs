...
use mcproto_rs::v1_15_2::{HandshakeSpec, PacketDirection, Packet578 as Packet, PlayDisconnectSpec, Id};
...
impl Proxy {
    pub fn new(config: Configuration) -> Result<Self> {
        ...
        let private_key = RSAPrivateKey::new(&mut rng, 1024).map_err(move |err| anyhow!("rsa gen error: {:?}", err))?;
    ...
    pub async fn listen_and_serve(&self) -> Result<()> {
        ...
        inner.logger.trace(format_args!("entering listen_and_serve with config {:?}", self.config()));
            ...
            let handle = tokio::spawn({
                ...
                async move {
                    ...
                    loop {
                        ...
                        InitialUpstreamHandler::new(connection, Self { inner: inner_arc.clone() }).spawn_handler();
                    }
                }
            });
    ...
    pub async fn players(&self) -> Vec<PlayerRef> {
        self.inner.players.read().await.by_id.iter().map(move |(_, value)| value.clone()).collect()
    }
    ...
    pub fn decrypt_token(&self, data: &[u8]) -> Result<Vec<u8>> {
        Ok(self.inner.private_key.decrypt(PaddingScheme::PKCS1v15, data)
    ...
    pub async fn add_new_player(&self, upstream: Bridge, handshake: HandshakeSpec, join_response: HasJoinedResponse) -> AddPlayerStatus {
...
impl ProxyPlayers {
    ...
    fn add_player(&mut self, proxy: Proxy, upstream: Bridge, handshake: HandshakeSpec, join_response: HasJoinedResponse) -> AddPlayerStatus {
...
impl Player {
    pub async fn connect_to_initial_downstream(&self, ) -> Result<()> {
        ...
        let initial_servers: Vec<_> = inner.proxy.config().servers.iter().filter(move |s| s.connect_to).cloned().collect();
        if initial_servers.is_empty() {
            return inner.disconnect_upstream(Chat::from_text("no server available for you to connect to...")).await;
        }

        let server = initial_servers.get(rand::random::<usize>() % initial_servers.len()).expect("server exists");
...
impl Drop for Player {
    fn drop(&mut self) {
        ...
        inner.proxy.logger().info(format_args!("player {} disconnected", inner.username));
    }
}

impl PlayerInner {
    async fn disconnect_upstream(&mut self, message: Chat) -> Result<()> {
        self.upstream.as_mut().expect("must be connected").write_packet(Packet::PlayDisconnect(PlayDisconnectSpec{
            reason: message
        })).await
    }
...
async fn forward_forever(mut source: ReadBridge, mut to: WriteBridge) -> Result<()> {
    tokio::spawn(async move {
        ...
        Ok(())
    }).await.map_err(move |join_err| anyhow::anyhow!("join err: {:?}", join_err))?
}

async fn downstream_connect(target: String, username: String, ip: String, id: UUID4, handshake: HandshakeSpec) -> Result<Bridge> {
    use mcproto_rs::v1_15_2::{PacketDirection::ClientBound, State::{Login, Play}, Packet578::{Handshake, LoginStart, LoginSetCompression, LoginSuccess, LoginEncryptionRequest}, LoginStartSpec, HandshakeNextState};

    let stream = TcpStream::connect(target).await?;
    let peer_addr = stream.peer_addr()?;
    let mut bridge = Bridge::initial(ClientBound, stream, peer_addr);
    bridge.write_packet(Handshake(HandshakeSpec{
        version: 578.into(),
    ...
    bridge.write_packet(LoginStart(LoginStartSpec{
        name: username,
    })).await?;
    loop {
